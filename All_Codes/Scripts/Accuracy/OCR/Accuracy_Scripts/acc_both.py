...
with open("<path to All_codes>/Scripts/Accuracy/OCR/xml_parsed_both/tem1.txt") as f:
    content_original = f.readlines()

content_original = [x.strip() for x in content_original]

with open("<path to All_codes>/Scripts/Accuracy/OCR/Output_Both/tem.txt") as f:
...
for i in range(0,len(content_mine)):
	tem = "<path to All_codes>/Scripts/Accuracy/OCR/xml_parsed_both/" + content_original[i]
        with open(tem) as f:
...
#	print orig
        mine_data = ""
	mine_data_full = ""
        tem = "<path to All_codes>/Scripts/Accuracy/OCR/Output_Both/" + content_mine[i]
        with open(tem) as f:
